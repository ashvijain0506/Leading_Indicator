...
Build: `sql/03_stage.sql` (tags every raw row with the first exclusion step it fails), `sql/04_clean.sql` (creates `clean_300a` with `recordable_cases`, `trir`, `dart`, `sector`, `size_band`), `sql/05_waterfall.sql`, `sql/06_overall.sql`, `sql/07_sector_rankings.sql`, `sql/08_size_band.sql`, `sql/09_state.sql`; results written to `output/tables/*.csv`; notebook sections 3–4.

*(Filenames as built. The plan above originally listed `03_clean.sql`, `04_drop_waterfall.sql`, `05_sector.sql`, `06_size_band.sql`, `07_state.sql`; inserting the staging step at 03 shifted everything after it by one, and §7 was already written against the real names.)*
